...
from common.opposite import get_opposite_direction, get_opposite_ew
...
def agent(observation, configuration):
    ...
    for worker in workers:
        if worker.can_build(game_state.map):
            ...
            acs.append(worker.move(get_opposite_ew(random.choice(['n','c','e','w']),player.team)))

    for cart in carts:
        if cart.can_build(game_state.map):
            acs.append(cart.build_city())
        elif cart.can_act():
            acs.append(cart.move(get_opposite_ew(random.choice(['n','c','e','w']),player.team)))
